...
const ChatbotModule = (() => {
  ...
  const State = {
    ...
    shouldDisplayJobTiles() {
      const jobRelatedOptions = ["job openings", "career opportunities", "recruitment"];
      return this.userSelectedOptions.some(option => jobRelatedOptions.includes(option.toLowerCase()));
    },
    ...
    save() {
      ...
      if (!this.shouldDisplayJobTiles()) {
        sessionStorage.removeItem("jobContainersHTML");
      }
    ...
    load() {
      ...
      const savedJobContainers = JSON.parse(sessionStorage.getItem("jobContainersHTML")) || [];
      ...
      if (savedState) {
        ...
        setTimeout(() => {
          ...
          document.querySelectorAll(".tile-link img").forEach((icon) => {
            if (icon.src.includes("arrow_up_right.svg")) {
              icon.remove();
            }
          });
        }, 100); ChatHandler.reattachChatMessageOptions();
        ...
        if (this.shouldDisplayJobTiles()) {
          savedJobContainers.forEach(jobContainerHTML => {
            if (!document.querySelector(".tile-container")) {
              Elements.chatBox.insertAdjacentHTML("beforeend", jobContainerHTML);
  ...
  const UIComponents = {
    ...
    createChatMessage(message, className) {
      ...
      if (className === "incoming") {
        ...
        const messageText = messageContent.querySelector("p") || messageContent.cloneNode(true);
    ...
    createJobTiles(jobs, container) {
      ...
      let existingContainers = JSON.parse(sessionStorage.getItem("jobContainersHTML")) || [];
      existingContainers.push(tileContainer.outerHTML);
      sessionStorage.setItem("jobContainersHTML", JSON.stringify(existingContainers));
    ...
    createTextMessage(message, container, isIncoming = false) {
      ...
      if (restoredMessages.includes(message)) {
      ...
      if (isIncoming && !restoredMessages.includes(message)) {
        ...
        this.typingInterval = setInterval(() => {
          ...
          const shouldAutoScroll = chatBox.scrollHeight - chatBox.clientHeight - chatBox.scrollTop < 100;
        ...
        // If the message isn't animated, just display it fully
        content.innerHTML = this.stripBlockElements(displayMessage);
        if (isTruncated) {
          this.addShowMoreButton(content, message, messageId);
        }
        ...
        if (isExpanded) {
          ...
          setTimeout(() => {
            ...
            if (messageBottom > chatBoxBottom) {
              chatBox.scrollTo({
                top: chatBox.scrollTop + (messageBottom - chatBoxBottom) + 50, // Scroll a bit extra for margin
                behavior: "smooth"
              });
            }
          }, 100);
    ...
    stripBlockElements(message) {
      ...
      //   li.innerHTML = `<strong>- </strong>${content}`;
      // });
      ...
      blockElements.forEach((tag) => {
        ...
        elements.forEach((el) => {
          // If it's an h2, preserve the strong tag with styling we just added
          if (tag === "h2") {
            const strongContent =
              el.querySelector("strong")?.innerHTML || el.innerHTML;
            const strongElement = document.createElement("strong");
            strongElement.style.fontSize = "18px";
            strongElement.textContent = strongContent;
            el.replaceWith(strongElement);
          } else {
    ...
    reattachLinkIcons(container) {
      ...
      links.forEach((link) => {
        if (link.classList.contains("tile-link")) return;
        ...
        // Create and append new icon
        const arrowIcon = document.createElement("img");
        arrowIcon.src = "assets/icons/arrow_up_right.svg";
        arrowIcon.alt = "External Link";
        arrowIcon.style.width = "16px";
        arrowIcon.style.height = "16px";
  ...
  const ChatHandler = {
    async handleChat() {
      ...
      Elements.chatBox.appendChild(
        UIComponents.createChatMessage(userInput, "outgoing")
      );
      Elements.chatBox.appendChild(UIComponents.createTimeStamp("outgoing"));
      State.userSelectedOptions.push(userInput.toLowerCase());
      Elements.chatBox.scrollTo(0, Elements.chatBox.scrollHeight);

      // Get and display bot response
      await this.fetchBotResponses(userInput.toLowerCase());
      await this.handleBotReply(userInput.toLowerCase());
      State.save();
    },
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          ...
          if (botResponse) {
            ...
            // logoImg.alt = "Chatbot Logo";
            // logoImg.classList.add("bot-logo-img");
            // botLogo.appendChild(logoImg);
            // messageWrapper.appendChild(botLogo);

            let messageContent = document.createElement("p");
            messageContent.classList.add("message-content");
            messageContent.setAttribute("data-full-message", botResponse.response);
            messageWrapper.appendChild(messageContent);

            Elements.chatBox.appendChild(messageElement);

            const fullMessage = botResponse.response;
            const maxMessageLength = CONSTANTS.MAX_MESSAGE_LENGTH;
            let index = 0;
            let showMoreTriggered = false;
            window.typingDone = false;

            const typingInterval = setInterval(() => {
              if (index >= fullMessage.length) {
                clearInterval(typingInterval);
                window.typingDone = true;
                return;
              }

              messageContent.innerHTML = UIComponents.stripBlockElements(fullMessage.slice(0, index + 1));
              UIComponents.reattachLinkIcons(messageContent);

              if (!showMoreTriggered && index >= maxMessageLength) {
                clearInterval(typingInterval);
                let existingShowMore = messageContent.querySelector(".show-more-btn");
                if (!existingShowMore) {
                  setTimeout(() => {
                    UIComponents.addShowMoreButton(messageContent, fullMessage);
            ...
            if (botResponse.chatMessageOptions?.length > 0) {
              setTimeout(() => {
                ChatHandler.showChatMessageOptions(botResponse.chatMessageOptions);
            ...
            if (botResponse.jobs?.length > 0) {
              ...
              }, CONSTANTS.TYPING_DELAY * 5);
            }

            Elements.chatBox.appendChild(UIComponents.createTimeStamp("incoming"));

            if (window.checkTypingDoneInterval) {
              clearInterval(window.checkTypingDoneInterval);
            }

            window.checkTypingDoneInterval = setInterval(() => {
              if (window.typingDone && botResponse.options?.length > 0) {
                ChatHandler.showOptions(botResponse.options);
                window.typingDone = false;
                clearInterval(window.checkTypingDoneInterval);
              }
            }, 100);
    ...
    handleOptionClick(option) {
      ...
      console.log("Current state before:", {
        option: lowerOption,
        inHistory: State.userSelectedOptions.includes(lowerOption),
        hasResponse: !!State.botResponses[lowerOption]
      });
      ...
      // Always add to user selections
      State.userSelectedOptions.push(lowerOption);
      ...
      Promise.all(animationPromises).then(() => {
        ...
        console.log("Before fetchBotResponses, response exists:", !!State.botResponses[lowerOption]);

        if (State.botResponses[lowerOption]) {
          console.log("Storing existing response temporarily");
          const tempResponse = { ...State.botResponses[lowerOption] };
          delete State.botResponses[lowerOption];

          this.fetchBotResponses(lowerOption)
            .then(() => {
              console.log("After fetchBotResponses, response exists:", !!State.botResponses[lowerOption]);

              if (!State.botResponses[lowerOption]) {
                console.log("Manually restoring response");
                State.botResponses[lowerOption] = tempResponse;
              }

              typingDone = false;
              return this.handleBotReply(lowerOption);
            })
            .then(() => {
              State.save();
              Elements.chatBox.scrollTo(0, Elements.chatBox.scrollHeight);
          ...
          this.fetchBotResponses(lowerOption)
            .then(() => {
              console.log("After fetchBotResponses (first time), response exists:", !!State.botResponses[lowerOption]);
              typingDone = false;
              return this.handleBotReply(lowerOption);
            })
            .then(() => {
    ...
    `;
      ...
      if (lastIncomingTimestamp) {
        ...
        lastIncomingTimestamp.scrollIntoView({ behavior: 'smooth', block: 'start' });
      } else {
  ...
  const initialize = () => {
    ...
    console.log(
      `Chatbot initialization complete (${stateLoaded ? "restored" : "new"
      } session)`
    );
  };
...
window.onload = function () {
  ...
  const showToastWithTypingAnimation = () => {
    const toastMessage = "Welcome, future designer! Need a hand? I've got you covered.";
      ...
      setTimeout(() => {
        ...
        const typingInterval = setInterval(function () {
          ...
          if (index === toastMessage.length) {
            ...
            // Start 5 second auto-hide timer after typing animation completes
            if (autoHideTimeout) {
  ...
  closeBtn.addEventListener("click", function (e) {
    ...
    //auto-hide timeout when manually closed
    if (autoHideTimeout) {
      clearTimeout(autoHideTimeout);
  ...
  toast.addEventListener("click", function () {
    //auto-hide timeout when toast is clicked
    if (autoHideTimeout) {
      clearTimeout(autoHideTimeout);
      autoHideTimeout = null;
    }
  ...
  chatbotToggle.addEventListener("click", function () {
    ...
    if (isChatbotOpen) {
      //auto-hide timeout when chatbot is opened
      if (autoHideTimeout) {
        clearTimeout(autoHideTimeout);
        autoHideTimeout = null;
      }
