...
class Supervisor:
    ...
    def understand(self, state):
        ...
        if mode is None:
            ...
            if question.lower() == "explore lollypop":
                return {'messages': messages, "next_node": "services"}
            if question.lower() == "start a project with us":
                return {'messages': messages, "next_node": "projects"}
            if question.lower() == "i'm looking for a job":
...
class MultiTenantGraph:
    ...
    def _post_processing(self, output):
        ...
        return [chatbot_answer, llm_free_options, chatMessageOptions, jobs]
...
if __name__ == "__main__":
    ...
    with open("graph_lollypop_v3.png", "wb") as png:
